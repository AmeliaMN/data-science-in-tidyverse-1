---
title: "NICAR 2020: Visualizing data with Tidyverse"
author: "Amelia McNamara"
---

## Thank you

These slides were (extremely lightly) modified from those used by [Andrew Ba Tran](https://twitter.com/abtran) at NICAR 2020. Thanks to Andrew for making them open-source so I could benefit! 
...
ggplot(ages, aes(x = actor, color = Genre)) +
  geom_bar()
```
...
ggplot(ages, aes(x = actor), fill = Genre) +
  geom_bar()
```
...
ggplot(ages, aes(x = actress_age, fill = actor)) +
...
```{r grouping2}
ggplot(ages, aes(x = actor_age, y = actress_age, color = actor, shape = Genre)) +
  geom_point()
```
...
```{r grouping3}
ggplot(ages, aes(x = actor_age, y = actress_age, color = actor, shape = actor)) +
  geom_point()
```
...
```{r grouping4}
ggplot(ages, aes(x = actor_age, y = actress_age, color = actor, size = budget)) +
  geom_point()
```
...
```{r coords}
ggplot(avg_age, aes(x = actor, y = average_age_diff)) +
