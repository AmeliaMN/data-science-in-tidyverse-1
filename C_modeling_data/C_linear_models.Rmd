...
To assess these conditions, we need to look at plots of residuals.
...
The other two conditions are independence (I call this the "thinking condition") and normality. We want our residuals to be normally distributed. There are a couple ways to check this.
...
```
To me, this looks to be non-normal. I can see a skew there. Statisticians like to look at a "QQ" or "quantile-quantile" plot to assess normality. 
...
In order to have a model we can trust, we need to fix these violations. There are rules of thumb to help, but almost always what you need is a log transformation. We'll use the `log()` function, which in R is the natural log.

```{r}
ggplot(elementary_schools) + geom_histogram(aes(x=parc_cpct))


lm(log(parc_cpct) ~ pct_low_inc + chi, data = elementary_schools) %>%
  augment() %>%
