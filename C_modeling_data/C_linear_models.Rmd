...
```{r}
ggplot(schools_data, aes(x = parc_cpct)) +
...
```{r}
ggplot(schools_data, aes(pct_black)) +
  geom_histogram(binwidth = 10)
```
...
If we feed a model through the augment() function, it will calculate fitted (or "predicted") values for us.
...
```{r}
multi_mod_outliers <- multi_mod %>%
  augment(elementary_schools) %>% 
  filter(.resid < -20 | .resid > 20)
multi_mod_outliers %>%
  arrange(.resid)
```
...
The other two conditions we can assess with a plot is normality. We want our residuals to be normally distributed. There are a couple ways to check this.
...
In order to have a model we can trust, we need to fix these violations. There are rules of thumb to help (searching for "Tukey's bulging rule" to learn more), but in many cases what you need is a log transformation. 

```{r}
skim(elementary_schools)
summary(low_inc_mod)
ggplot(elementary_schools) + geom_histogram(aes(x=sqrt(parc_cpct)))
plot(multi_mod)

lm(sqrt(parc_cpct) ~ pct_low_inc + chi, data = elementary_schools) %>%
  augment() %>%
...
We can't interpret our coefficients the same way anymore. In fact, these numbers represent a "space" that is hard for our brains to reason about-- log-odds space. We would rather think in odds space or probability space. [Amelia-- add link to 291 lab]
